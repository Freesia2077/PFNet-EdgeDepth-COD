...
ckpt_path = './ckpt'
exp_name = 'PFNet_RGBD' # 建议改个名字区分一下
...
def main():
    ...
    if args['optimizer'] == 'Adam':
        optimizer = optim.Adam([
            ...
            {'params': [param for name, param in net.named_parameters() if name[-4:] != 'bias'], 'lr': 1 * args['lr'], 'weight_decay': args['weight_decay']}
        ])
    else:
        optimizer = optim.SGD([
            {'params': [param for name, param in net.named_parameters() if name[-4:] == 'bias'], 'lr': 2 * args['lr']},
...
def train(net, optimizer):
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        ...
        for data in train_iterator:
            ...
            # 解包数据：增加 depths
            inputs, labels, depths = data
            batch_size = inputs.size(0)
            
            inputs = Variable(inputs).cuda(device_ids[0])
            labels = Variable(labels).cuda(device_ids[0])
            depths = Variable(depths).cuda(device_ids[0])

            optimizer.zero_grad()

            # 模型前向传播：增加 depths 参数
            predict_1, predict_2, predict_3, predict_4 = net(inputs, depths)
