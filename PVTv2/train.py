...
log_path = os.path.join(ckpt_path, exp_name, now_str + '.txt')
...
def main():
    ...
    # 为 Transformer backbone 设置更小的学习率（这是 Transformer 训练的最佳实践）
    # 参考：通常 backbone 学习率是 decoder 的 0.1-0.2 倍
    backbone_lr_ratio = 0.1  # PVTv2 backbone 学习率是 decoder 的 0.1 倍
    
    if args['optimizer'] == 'Adam':
        print("Adam")
        # 分离 backbone 和 decoder 参数
        backbone_params = []
        decoder_params = []
        bias_params = []
        ...
        optimizer = optim.Adam([
            {'params': bias_params, 'lr': 2 * args['lr']},
            {'params': decoder_params, 'lr': 1 * args['lr'], 'weight_decay': args['weight_decay']},
            {'params': backbone_params, 'lr': backbone_lr_ratio * args['lr'], 'weight_decay': args['weight_decay']}
        ])
        print("Backbone learning rate: {:.6f}, Decoder learning rate: {:.6f}".format(
            backbone_lr_ratio * args['lr'], args['lr']))
    else:
        print("SGD")
        # 分离 backbone 和 decoder 参数
        backbone_params = []
    ...
    train(net, optimizer, backbone_lr_ratio)
    writer.close()

def train(net, optimizer, backbone_lr_ratio=0.1):
    curr_iter = 1
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        ...
        for data in train_iterator:
            if args['poly_train']:
                ...
                backbone_base_lr = backbone_lr_ratio * base_lr
                # 更新学习率：bias, decoder, backbone
                optimizer.param_groups[0]['lr'] = 2 * base_lr  # bias
                optimizer.param_groups[1]['lr'] = 1 * base_lr  # decoder
                optimizer.param_groups[2]['lr'] = 1 * backbone_base_lr  # backbone
